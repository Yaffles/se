    ...
    if (!exam) {
      ...
      `;
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <!-- girra exam -->
          <a href="?exam=girraween" class="block bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition border border-gray-200">
          ...
          <!-- hsc_familiarisation exam -->
          <a href="?exam=hsc_familiarisation" class="block bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition border border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">HSC Familiarisation Questions</h2>
            <p class="text-gray-600 mt-2">HSC Software Engineering familiarisation questions.</p>
          </a>
          <!-- hsc_sample_exam exam -->
          <a href="?exam=hsc_sample_exam" class="block bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition border border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">HSC Sample Exam</h2>
            <p class="text-gray-600 mt-2">HSC Software Engineering sample exam questions.</p>
          </a>
        </div>
      `;
    } else {
        ...
        // add home / back button to top header section and a link to answers
        document.querySelector('header').innerHTML += `
          <a href="/" class="text-indigo-600 hover:underline mt-4 inline-block">← Back to exams</a><br>
          <a href="/answers/${exam}_answers.pdf" target="_blank" class="text-rose-600 hover:underline mt-4 inline-block">View Answers</a>
        `;
            ...
            fetch(examFiles[exam])
                ...
                .then(jsonData => {
                    ...
                    <div class="relative" id="slider-${questionData.id}">
                      ...
                      `).join('')}
                      <button type="button" class="slider-arrow prev" onclick="changeSlide(-1, 'slider-${questionData.id}')">&#10094;</button>
                      <button type="button" class="slider-arrow next" onclick="changeSlide(1, 'slider-${questionData.id}')">&#10095;</button>
                      </div>`;
                  ...
                  <div id="${outputId}" class="overflow-x-auto w-full p-4 bg-gray-100 rounded-md text-sm text-gray-800"></div>
                                ...
                                setTimeout(async () => {
                                    ...
                                    if (answerParams.datasetSetupQuery) {
                                        try {
                                            db.run(answerParams.datasetSetupQuery);
                                        } catch (err) {
                                            console.error("Dataset setup failed:", err);
                                        }
                                    }
                                    document.getElementById(runBtnId).addEventListener("click", () => {
                                        ...
                                        try {
                                            ...
                                            result[0].values.forEach(row => {
                                                html += "<tr>";
                                                row.forEach(val => {
                                                    html += `<td class="border border-gray-300 px-2 py-1">${val}</td>`;
                                                });
                                                html += "</tr>";
                                            });
                                            html += "</tbody></table>";
                                ...
                                answerHTML += `
                <div class="mt-4">
                  <iframe src="https://excalidraw.com/?embed=1" width="100%" height="600" style="border:1px solid #e2e8f0; border-radius: 8px;" tabindex="-1" onload="this.contentWindow.blur(); this.blur();"></iframe>
                </div>`;
                            }
                            answerHTML += '</div>';
